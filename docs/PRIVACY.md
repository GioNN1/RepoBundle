...
RepoBundle reads local repository files to create local Markdown output. The extension itself does not upload repository contents to an external service.

RepoBundle may execute local Git commands when Git-backed discovery or snapshot metadata is requested by its normal operation.
